    ...
    <div ref={chatWrapper} className={styles.main}>
      {user.messages[interlocutor] &&
        user.messages[interlocutor].discussion.map((item, index) => (
          <div
            key={index}
            className={`${styles.chatBaloon} ${
              item.author === user._id && styles.mine
            }`}
          >
            <Link to="/userdetails" state={item.author}>
              <p className={styles.author}>
                {item.author === user._id ? (
                  ...
                  <>
                    {user.messages[interlocutor].user.name}{" "}
                    {user.messages[interlocutor].user.surname}
                  </>
                )}
              </p>
            </Link>
